...
//                    dd($row);
        ...
        return view("frontend.cart", compact('cart_table', 'conversion_rate', 'oot'));
...
//        $cart->vender_id = $product->store->user->id;
    ...
    public function removesimpleproduct($id)
    {
        if (!auth()->check()){
            ...
            foreach (Session::get('guest_cart') as $key=> $del){
//                print $key;
                if ($key == 0){
                    if ($del != $id){
//                        return 'hello';
                        Session::put('guest_cart', [$del]);
                    }else{
                        Session::pull('guest_cart');
                    }
                }else{
//                    return 'hello 2';
                    if (Session::has('guest_cart')){
                        if ($del != $id){
                            Session::push('guest_cart', $del);
                        }
                    }else{
                        if ($del != $id){
                            Session::put('guest_cart', [$del]);
